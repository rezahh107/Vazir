...
await recorder.record('frontend_view_table_search_pagination', async () => {
  ...
  await expectVazirmatn(container, 'GravityView frontend container'); await expectNotVazirmatn(page.locator('#vf-view-excluded'), 'GravityView profile exclusion fixture', /monospace/i);
  const searchForm = page.locator('form.gv-widget-search').first(); await searchForm.waitFor({ state: 'visible', timeout: 30000 });
  const search = searchForm.locator('input[type="search"], input[type="text"]').first(); await expectVazirmatn(search, 'GravityView search input');
  const submit = searchForm.locator('.gv-search-button').first(); await expectVazirmatn(submit, 'GravityView search button');
  await search.fill('آلفا'); await Promise.all([page.waitForLoadState('domcontentloaded'), submit.click()]);
  await page.locator('.gv-container').first().waitFor({ state: 'visible' }); await expectVazirmatn(page.locator('.gv-container').first(), 'GravityView filtered result state');
  ...
  if (await pagination.count()) await expectVazirmatn(pagination, 'GravityView pagination'); else results.claim_ceiling.pagination = 'NOT_PROVEN: configured result state did not render pagination';
});
