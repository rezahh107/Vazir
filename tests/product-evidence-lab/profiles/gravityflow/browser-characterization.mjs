...
await recorder.record('admin_current_inbox', async () => {
  ...
  await expectVazir(inbox, 'Gravity Flow admin inbox');
  const search = page.locator('input.gflow-inbox__search, .gravityflow-entry-table__header-search-input').first(); if (await search.count()) await expectVazir(search, 'Gravity Flow inbox search control');
...
await recorder.record('frontend_inbox_shortcode', async () => {
  ...
  await expectVazir(inbox, 'Gravity Flow frontend inbox'); await expectNotVazir(page.locator('#vf-flow-excluded'), 'Gravity Flow profile exclusion fixture', /monospace/i);
});
await recorder.record('gravity_forms_prerequisite_still_operational', async () => {
  ...
  await expectVazir(wrapper, 'Gravity Forms prerequisite with Gravity Flow active'); await expectVazir(page.locator(`#input_${manifest.form_id}_1`), 'Gravity Forms prerequisite control with Gravity Flow active');
});
